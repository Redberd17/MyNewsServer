...
public class NewsController {
  ...
  public ResponseEntity<NewsResponse> getAllNewsPaginated(
          @RequestParam(name = Constants.PAGE_SIZE) Long pageSize,
          @RequestParam(name = Constants.PAGE) Long page) {
    if (page <= 0) {
      throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Page can not be <= 0");
    }
    if (pageSize < 0) {
      throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Page size can not be < 0");
    }
    NewsResponse newsResponse = newsService.getNewsPaginated(pageSize, page);
    if (CollectionUtils.isEmpty(newsResponse.getArticles())) {
      return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    } else {
